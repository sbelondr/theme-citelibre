...
function addr_search(genericAttributes_id) {
	...
	var resultList = "";
	if (data.length > 0){
		resultList += '<li class="no-hover text-gray"> <small><i class="fas fa-info-circle"></i> Cliquer sur une adresse dans la liste ci-dessous pour la sélectionner</small></li>';
		$.each(data, function(key, val) {
			resultList += '<li onclick="chooseAddr(' + val.lat + ', ' + val.lon + ',this,' + genericAttributes_id + ');return false;">' + val.display_name + '</li>';
		});
	} else {
		resultList += '<li class="text-danger no-hover"> <i class="fas fa-exclamation-triangle"></i><strong>Aucune adresse pour cette recherche</strong></li>';
	}
